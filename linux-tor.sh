...
if args.upload:
	...
	if args.encrypt:
		...
		encrypt_file=ufile
		encrypted=1
	response = requests.post('http://7b42twezybs23hrr.onion/upload.php', proxies=proxies, files={'file': (upload_filename, open(ufile, 'rb'), 'application/octet-stream', {'Expires': '0'})})
...
if args.meta:
	...
	try:
		with open(file, 'rt') as csvfile:
			...
			for row in csvreader:
				if row[1] == args.meta:
					response = requests.post('http://7b42twezybs23hrr.onion/meta.php',proxies=proxies, data={'filename':args.meta,'control':row[3]})
...
if args.rm:
	...
	try:
		with open(file, 'rt') as csvfile:
			...
			for row in csvreader:
				if row[1] == args.rm:
					response = requests.post('http://7b42twezybs23hrr.onion/rm.php',proxies=proxies, data={'filename':args.rm,'control':row[3]})
					found=True
		if not found:
			print("File Not Found")
		else:
...
if args.download:
	...
	try:
		...
		with open(file, 'rt') as csvfile:
			...
			for row in csvreader:
				if row[1] == args.download:
					response = requests.post('http://7b42twezybs23hrr.onion/download.php',proxies=proxies, data={'filename':args.download,'control':row[3]})
			                salt=":&\WRdDv6'MvK{8C"
			                N = 1024
			                r = 1
			                p = 1
